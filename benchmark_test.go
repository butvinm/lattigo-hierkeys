...
// Realistic sparse target rotation sets mimicking FHE workloads: small positives (BSGS),
// powers of 2 (bootstrapping), convolution strides, negative rotations (values near N/2).
...
//
// As a consequence, `held` is NOT "the working set during this phase" — it is "what's still reachable from variables used after this call".
// For expansion phases, that means held ≈ the keys you copied into IntermediateKeys plus the previous level's masters;
// the stepping stones in exp.entries get dropped automatically by Go's liveness.
// Use peak when sizing memory; use the peak/held delta to see how much of the phase's footprint is transient.
//
// For memory metrics to be meaningful, run with -benchtime=1x;
// with higher iteration counts, ReportMetric overwrites per iteration but values should be similar across runs.
func measurePhase(b *testing.B, phase string, phaseStart *time.Time) {
	...
	// Reset the timer for the next phase, AFTER the forced GC so the measurement overhead is not counted toward the next phase.
	*phaseStart = time.Now()
}
