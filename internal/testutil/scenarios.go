// Package testutil provides shared parameter scenarios for tests and
// benchmarks. Not part of the public API — importable only within this module.
package testutil

// Scenario is a concrete 128-bit-secure parameter set for a given ring degree.
// Used by both benchmarks (256 target rotations, full sparse set) and tests
// (reduced target set, production-params smoke coverage).
//
...
// each digit holds PCount consecutive Q primes regardless of size. Noise
// blows up by 2^(max_digit_bits − P_bits) when P bits < max digit bits.
...
type Scenario struct {
	...
	// KG+ 3-level additions. LogPHK3 primes are P^(1) at Levels[1]; LogPExtra
	// primes are P^(2) at Levels[2] (top). Sized for dnum=1 at top when possible.
	LogPHK3   []int
	LogPExtra []int
}
...
// Scenarios covers LogN=14/15/16 at 128-bit security. Shared between benchmarks
// and tests so production params are exercised by both.
...
// ReducedTestTargets returns a small set of target rotations that exercise
// different decomposition chain lengths. Used by tests to verify end-to-end
// correctness at production scale without the cost of 256-target benchmarks.
//
