...
type testContext struct {
	params     Parameters
	kgen       *KeyGenerator
	sk         *rlwe.SecretKey // at master level
...
func TestLLKN(t *testing.T) {
	...
	for _, paramsLit := range testInsecure {
		...
		params, err := NewParameters(paramsEval, paramsLit.LogPMaster)
		require.NoError(t, err)
...
func testKeyGenerator(tc *testContext, t *testing.T) {
	...
	t.Run(testString(params, "KeyGenerator"), func(t *testing.T) {
		...
		t.Run("GenSecretKeyNew", func(t *testing.T) {
			...
			require.NotNil(t, sk)
			require.Equal(t, params.Master.QCount(), sk.LevelQ()+1)
			require.Equal(t, params.Master.PCount(), sk.LevelP()+1)
		})
